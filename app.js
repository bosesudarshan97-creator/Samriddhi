...
async function loadRemoteState() {
	...
	state.loans = loans.map((loan) => ({ id: loan.id, memberId: loan.member_id, principal: Number(loan.principal), date: loan.loan_date, payments: payments.filter((payment) => payment.loan_id === loan.id).map((payment) => ({ id: payment.id, amount: Number(payment.amount), type: payment.payment_type || 'auto', date: payment.payment_date, createdAt: payment.created_at })) }));
...
async function ensureCurrentUserMember() { const email = currentUser.email?.toLowerCase() || ''; const name = currentUser.user_metadata?.full_name || email.split('@')[0] || 'Fund member'; const phone = currentUser.user_metadata?.phone || currentUser.phone || 'Not provided'; const { data: linkedMember, error: linkedLookupError } = await supabaseClient.from('members').select('id').eq('user_id', currentUser.id).maybeSingle(); if (linkedLookupError) throw linkedLookupError; if (linkedMember) return; if (email) { const { data: existingMember, error: emailLookupError } = await supabaseClient.from('members').select('id, user_id').ilike('email', email).maybeSingle(); if (emailLookupError) throw emailLookupError; if (existingMember) { if (existingMember.user_id && existingMember.user_id !== currentUser.id) throw new Error('This email is linked to another member account. Contact the administrator.'); const { error } = await supabaseClient.from('members').update({ user_id: currentUser.id, email, active: true }).eq('id', existingMember.id); if (error) throw error; return; } } const { error } = await supabaseClient.from('members').insert({ user_id: currentUser.id, email, name, phone, active: true }); if (error && error.code !== '23505') throw error; }
...
document.addEventListener('click', async (event) => { const open = event.target.closest('[data-open-modal]'); const close = event.target.closest('[data-close-modal]'); const tab = event.target.closest('[data-tab]'); const payment = event.target.closest('[data-payment]'); const historyToggle = event.target.closest('[data-payment-history]'); const memberWhatsApp = event.target.closest('[data-whatsapp]'); const loanWhatsApp = event.target.closest('[data-whatsapp-loan]'); const edit = event.target.closest('[data-edit-member]'); const remove = event.target.closest('[data-delete-member]'); if (open) openModal(open.dataset.openModal); if (close) $(`#${close.dataset.closeModal}`).close(); if (tab) { document.querySelectorAll('.tab, .tab-panel').forEach((element) => element.classList.remove('is-active')); tab.classList.add('is-active'); $(`[data-panel="${tab.dataset.tab}"]`).classList.add('is-active'); } if (payment) { $('#payment-loan-id').value = payment.dataset.payment; openModal('payment-modal'); } if (historyToggle) { const panel = $(`[data-payment-history-panel="${historyToggle.dataset.paymentHistory}"]`); const expanded = historyToggle.getAttribute('aria-expanded') === 'true'; historyToggle.setAttribute('aria-expanded', String(!expanded)); panel.hidden = expanded; } if (edit) { const member = state.members.find((item) => item.id === edit.dataset.editMember); if (member) { $('#edit-member-id').value = member.id; $('#edit-member-name').value = member.name; $('#edit-member-phone').value = member.phone === 'Not provided' ? '' : member.phone; openModal('edit-member-modal'); } } if (memberWhatsApp) { const member = state.members.find((item) => item.id === memberWhatsApp.dataset.whatsapp); window.open(whatsappUrl(member.phone, memberMessage(member.id)), '_blank', 'noopener'); } if (loanWhatsApp) { const loan = state.loans.find((item) => item.id === loanWhatsApp.dataset.whatsappLoan); const member = state.members.find((item) => item.id === loan.memberId); window.open(whatsappUrl(member.phone, loanMessage(loan)), '_blank', 'noopener'); } if (remove && window.confirm('Remove this member? Existing loan records will remain.')) { try { if (supabaseClient) await removeMember(remove.dataset.deleteMember); else { state.members = state.members.filter((member) => member.id !== remove.dataset.deleteMember); saveLocalState(); render(); } } catch (error) { showDataError(error); } } });
