...
function loanUsageClass(loan) { const limitAtIssue = Number(loan.eligibleLimit) || memberContributionCeiling(loan.memberId); if (limitAtIssue <= 0) return 'loan-usage--red'; const usagePercent = (Number(loan.principal) / limitAtIssue) * 100; if (usagePercent <= 20) return 'loan-usage--green'; if (usagePercent <= 40) return 'loan-usage--yellow'; if (usagePercent <= 50) return 'loan-usage--orange'; return 'loan-usage--red'; }
...
function renderLoans() {
	...
	function renderLoanGroups(target, sourceLoans) {
		...
		loans.slice().reverse().forEach((loan) => {
			...
			const usageClass = loanUsageClass(loan);
