...
function memberLoanUsagePercent(memberId) { const limit = memberContributionCeiling(memberId); const totalBorrowed = state.loans.filter((loan) => loan.memberId === memberId).reduce((total, loan) => total + Number(loan.principal), 0); return limit > 0 ? (totalBorrowed / limit) * 100 : 0; }
...
function renderLoans() {
	...
	function renderLoanGroups(target, sourceLoans) {
		...
		group.innerHTML = `<div class="loan-member-heading"><div><h3 class="loan-usage-name ${memberUsageClass}">${escapeHtml(memberName(memberId))} <span class="loan-usage-percent">${usagePercent.toFixed(1)}%</span></h3><small>${loans.length} ${isHistory ? 'closed' : 'active'} loan${loans.length === 1 ? '' : 's'}</small></div><strong>${formatMoney(memberDue)} total due</strong></div>`;
