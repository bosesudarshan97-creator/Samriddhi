...
async function inviteMember(record) { const { data, error } = await supabaseClient.functions.invoke('invite-member', { body: record }); if (error) throw error; if (data?.error) throw new Error(data.error); await refreshRemoteState(); }
...
function isPasswordLink() { return /(?:^|&)type=(?:invite|recovery)(?:&|$)/.test(window.location.hash.slice(1)); }
async function loadAuthState() { if (!supabaseClient) { applyAuthState(); return; } const { data: { session } } = await supabaseClient.auth.getSession(); if (isPasswordLink() && session) { currentUser = session.user; authMode = 'password'; applyAuthState(); } else await setSession(session); supabaseClient.auth.onAuthStateChange((_event, nextSession) => { if (_event === 'PASSWORD_RECOVERY' || (isPasswordLink() && nextSession)) { currentUser = nextSession?.user || null; authMode = 'password'; applyAuthState(); } else setSession(nextSession); }); }
...
$('#password-form').addEventListener('submit', async (event) => { event.preventDefault(); if (!supabaseClient) return; const errorElement = $('#password-error'); errorElement.hidden = true; if ($('#new-password').value !== $('#confirm-password').value) { errorElement.textContent = 'Passwords do not match.'; errorElement.hidden = false; return; } const { error } = await supabaseClient.auth.updateUser({ password: $('#new-password').value }); if (error) { errorElement.textContent = error.message; errorElement.hidden = false; return; } window.history.replaceState({}, document.title, window.location.pathname); authMode = 'sign-in'; $('#auth-error').textContent = 'Password saved. You can now sign in with your email and password.'; $('#auth-error').hidden = false; await supabaseClient.auth.signOut(); currentUser = null; applyAuthState(); event.target.reset(); });
