...
function memberLoanUsageClass(memberId) { const limit = memberContributionCeiling(memberId); const totalBorrowed = state.loans.filter((loan) => loan.memberId === memberId).reduce((total, loan) => total + Number(loan.principal), 0); if (limit <= 0) return 'loan-usage--red'; const usagePercent = (totalBorrowed / limit) * 100; if (usagePercent <= 20) return 'loan-usage--green'; if (usagePercent <= 40) return 'loan-usage--yellow'; if (usagePercent <= 50) return 'loan-usage--orange'; return 'loan-usage--red'; }
...
function renderLoans() {
	...
	function renderLoanGroups(target, sourceLoans) {
		...
		group.innerHTML = `<div class="loan-member-heading"><div><h3 class="loan-usage-name ${memberUsageClass}">${escapeHtml(memberName(memberId))}</h3><small>${loans.length} ${isHistory ? 'closed' : 'active'} loan${loans.length === 1 ? '' : 's'}</small></div><strong>${formatMoney(memberDue)} total due</strong></div>`;
