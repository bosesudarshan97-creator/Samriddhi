...
async function loadRemoteState() {
	...
	state.members = members.map((member) => ({ id: member.id, name: member.name, phone: member.phone, active: member.active, userId: member.user_id }));
...
async function ensureCurrentUserMember() { if (currentRole !== 'admin') return; const { data: existing, error: lookupError } = await supabaseClient.from('members').select('id').eq('user_id', currentUser.id).maybeSingle(); if (lookupError) throw lookupError; if (existing) return; const name = currentUser.user_metadata?.full_name || currentUser.email?.split('@')[0] || 'Fund administrator'; const phone = currentUser.phone || 'Not provided'; const { error } = await supabaseClient.from('members').insert({ user_id: currentUser.id, name, phone, active: true }); if (error && error.code !== '23505') throw error; }
