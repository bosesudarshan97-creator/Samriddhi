...
async function ensureCurrentUserMember() { if (currentRole !== 'admin') return; const email = currentUser.email?.toLowerCase() || ''; const name = currentUser.user_metadata?.full_name || email.split('@')[0] || 'Fund administrator'; const { data: existing, error: lookupError } = await supabaseClient.from('members').select('id, user_id, email').or(`user_id.eq.${currentUser.id},email.eq.${email}`).maybeSingle(); if (lookupError) throw lookupError; if (existing) { if (existing.user_id !== currentUser.id || existing.email !== email) { const { error } = await supabaseClient.from('members').update({ user_id: currentUser.id, email }).eq('id', existing.id); if (error) throw error; } return; } const phone = currentUser.phone || 'Not provided'; const { error } = await supabaseClient.from('members').insert({ user_id: currentUser.id, email, name, phone, active: true }); if (error && error.code !== '23505') throw error; }
...
document.addEventListener('click', async (event) => { const open = event.target.closest('[data-open-modal]'); const close = event.target.closest('[data-close-modal]'); const tab = event.target.closest('[data-tab]'); const payment = event.target.closest('[data-payment]'); const memberWhatsApp = event.target.closest('[data-whatsapp]'); const loanWhatsApp = event.target.closest('[data-whatsapp-loan]'); const edit = event.target.closest('[data-edit-member]'); const remove = event.target.closest('[data-delete-member]'); if (open) openModal(open.dataset.openModal); if (close) $(`#${close.dataset.closeModal}`).close(); if (tab) { document.querySelectorAll('.tab, .tab-panel').forEach((element) => element.classList.remove('is-active')); tab.classList.add('is-active'); $(`[data-panel="${tab.dataset.tab}"]`).classList.add('is-active'); } if (payment) { $('#payment-loan-id').value = payment.dataset.payment; openModal('payment-modal'); } if (edit) { const member = state.members.find((item) => item.id === edit.dataset.editMember); if (member) { $('#edit-member-id').value = member.id; $('#edit-member-name').value = member.name; $('#edit-member-phone').value = member.phone === 'Not provided' ? '' : member.phone; openModal('edit-member-modal'); } } if (memberWhatsApp) { const member = state.members.find((item) => item.id === memberWhatsApp.dataset.whatsapp); window.open(whatsappUrl(member.phone, memberMessage(member.id)), '_blank', 'noopener'); } if (loanWhatsApp) { const loan = state.loans.find((item) => item.id === loanWhatsApp.dataset.whatsappLoan); const member = state.members.find((item) => item.id === loan.memberId); window.open(whatsappUrl(member.phone, loanMessage(loan)), '_blank', 'noopener'); } if (remove && window.confirm('Remove this member? Existing loan records will remain.')) { try { if (supabaseClient) await removeMember(remove.dataset.deleteMember); else { state.members = state.members.filter((member) => member.id !== remove.dataset.deleteMember); saveLocalState(); render(); } } catch (error) { showDataError(error); } } });
