...
async function setSession(session) { currentUser = session?.user || null; currentRole = 'member'; if (currentUser) { const { data } = await supabaseClient.from('profiles').select('role').eq('id', currentUser.id).maybeSingle(); currentRole = data?.role === 'admin' ? 'admin' : 'member'; try { await migrateLocalState(); await loadRemoteState(); subscribeToLedger(); render(); } catch (error) { showDataError(error); } } applyAuthState(); }
