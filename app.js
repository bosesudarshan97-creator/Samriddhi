...
async function ensureCurrentUserMember() { if (currentRole !== 'admin') return; const email = currentUser.email?.toLowerCase() || ''; const name = currentUser.user_metadata?.full_name || email.split('@')[0] || 'Fund administrator'; const { data: existing, error: lookupError } = await supabaseClient.from('members').select('id, user_id').eq('user_id', currentUser.id).maybeSingle(); if (lookupError) throw lookupError; if (existing) return; const phone = currentUser.phone || 'Not provided'; const { error } = await supabaseClient.from('members').insert({ user_id: currentUser.id, name, phone, active: true }); if (error && error.code !== '23505') throw error; }
