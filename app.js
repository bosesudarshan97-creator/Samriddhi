...
function memberLoanLimit(memberId) { const contributionTotal = (state.contributions || []).filter((item) => item.memberId === memberId).reduce((total, item) => total + toPaise(item.amount), 0); const loans = state.loans.filter((loan) => loan.memberId === memberId); const issued = loans.reduce((total, loan) => total + toPaise(loan.principal), 0); const repayments = loans.reduce((total, loan) => total + loan.payments.reduce((sum, payment) => sum + toPaise(payment.amount), 0), 0); return fromPaise(Math.max(0, contributionTotal * 2 - issued + repayments)); }
